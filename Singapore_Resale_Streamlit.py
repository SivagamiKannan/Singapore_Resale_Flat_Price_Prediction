...
def predict_price(year,town,flat_type,flr_area_sqm,flat_model,stry_start,stry_end,re_les_year,
    ...
    lease_coms_dt= int(les_coms_dt)


    with open(r"Resale_flat_price_model.pkl","rb") as f:
...
if select == "Home":
    ...
    st.subheader(":rainbow[**The Singapore resale flat price prediction application helps in predicting the resale price of the flats based on different criteria in Singapore.**]")
    st.subheader(":rainbow[**It aims to assist both potential buyers and sellers in estimating the resale value of a flat.**]")
    st.write("")
    img= Image.open(r"C:\Users\sivag\DS\Datasets\Singapore_flats.jpg")
    st.image(img)
    ...
    button= st.button("Predict the Resale Price", use_container_width= True)
    ...
    if button:
        ...
        st.write("## :green[**The Predicted Resale Price is :**]",predicted_price)
